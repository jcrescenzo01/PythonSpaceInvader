...
enemyX = random.randint(0, 735)  # will 800 work poorly?
enemyY = random.randint(50, 150)
enemyX_change = 0.3  # for moving enemy left and right, etc
enemyY_change = 40  # for y axis enemy movment, set to 40 so we can move it down in the loop later
...
def enemy(x, y):  # player remade into an enemy
...
while running:  # infinite loop so it stays open, but it will hang if theres no quit functionality!
    ...
    if enemyX <= 0:
        enemyX_change = 0.3  # when it hits the left side start increasing the x value and
        # go the other way, note it doesnt set it to 0 since no one is
        # controlling this things movement but the compiler
        enemyY += enemyY_change  # increment ychange to move it down when border is hit
    elif enemyX >= 736:  # 800 would stop at the left edge of the ship, which would look bad
        enemyX_change = -0.3  # same as above
    ...
    # Collision
    collision = isCollision(enemyX, enemyY, bulletX, bulletY)  # stores value of true and false in collision
    # now what do we want when collision occurs?
    if collision:  # if collision is true
        bulletY = 480  # reset bullet to its starting point
        bullet_state = "ready"  # change the state since the bullet isnt shown anymore
        score += 1  # increase score by 1 each time we hit
        print(score)
        # now we need to get the enemy to respawn upon being hit, up to the enemy block to take stuff!
        enemyX = random.randint(0, 735)
        enemyY = random.randint(50, 150)  # now the enemy will respawn as it should
